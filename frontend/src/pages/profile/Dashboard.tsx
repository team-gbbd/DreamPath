...
const MBTI_DETAILS: Record<string, { title: string; description: string }> = {
  ESTJ: {
    title: "체계적인 리더",
    description: "명확한 목표와 조직적인 실행을 중시하는 유형입니다.",
  },
  ENTJ: {
    title: "전략가 리더",
    description: "비전을 세우고 팀을 이끄는 데 강한 능력을 보입니다.",
  },
  ENFJ: {
    title: "공감형 리더",
    description: "타인의 감정을 이해하고 협업을 이끄는 힘이 있습니다.",
  },
  ENFP: {
    title: "아이디어 메이커",
    description: "새로운 가능성을 발견하고 사람들에게 영감을 줍니다.",
  },
  ESTP: {
    title: "실행형 모험가",
    description: "빠르게 상황을 파악하고 과감히 행동합니다.",
  },
  ESFP: {
    title: "에너지 메이커",
    description: "주변에 활력을 전하며 실용적인 해결책을 찾습니다.",
  },
  ESFJ: {
    title: "협력형 서포터",
    description: "타인을 돕고 팀워크를 중시합니다.",
  },
  ENTP: {
    title: "창의적 토론가",
    description: "새로운 관점을 제시하고 논리로 설득합니다.",
  },
  ISTJ: {
    title: "신중한 관리자",
    description: "책임감 있게 일을 완수하고 안정성을 제공합니다.",
  },
  ISFJ: {
    title: "세심한 보조자",
    description: "섬세한 관찰력과 헌신으로 신뢰를 줍니다.",
  },
  INFJ: {
    title: "통찰형 조언자",
    description: "깊은 통찰력으로 의미 있는 방향을 제시합니다.",
  },
  INTJ: {
    title: "전략적 설계자",
    description: "장기 계획을 세우고 독창적인 방식으로 실행합니다.",
  },
  ISTP: {
    title: "문제 해결사",
    description: "실용적인 분석과 손재주로 난제를 해결합니다.",
  },
  INTP: {
    title: "이론가",
    description: "논리적 사고와 분석을 통해 새로운 개념을 탐구합니다.",
  },
  INFP: {
    title: "이상주의자",
    description: "가치를 중시하며 사람들에게 긍정적인 변화를 촉진합니다.",
  },
  ISFP: {
    title: "감성형 크리에이터",
    description: "자유롭고 따뜻한 태도로 조화를 추구합니다.",
  },
};
...
export default function NewDashboard() {
  ...
  const fetchProfileData = useCallback(
    async (targetUserId: number, options: FetchOptions = {}) => {
      ...
      const response = await fetch(
        `${BACKEND_BASE_URL}/api/profiles/${targetUserId}`,
        { signal }
      );
    ...
    try {
      ...
      if (statusRes.data?.ready && statusRes.data?.vectorId) {
        ...
        if (Array.isArray(majorsList)) {
          setTopMajors(
            majorsList.slice(0, 3).map((major: any, index: number) => ({
              rank: index + 1,
              title:
                major.title ||
              ...
              tag:
                major.metadata?.lClass ||
                major.metadata?.field ||
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "dreampath-auth-change",
      authHandler as EventListener
    );
  ...
  const personalityChartData = useMemo(() => {
    ...
    const traitSource =
      ...
      typeof (personalityJson as Record<string, unknown>).traits === "object"
        ? ((personalityJson as Record<string, unknown>).traits as Record<
            string,
            unknown
          >)
        : personalityJson;

    const entries = Object.entries(traitSource).filter(
  ...
  const valuesJson = useMemo(
    () =>
      safeParseJson<Record<string, unknown>>(
        analysisData?.values ?? profileData?.values
      ),
  ...
  const valuesChartData = useMemo(() => {
    ...
    const valueSource =
      (valuesJson as Record<string, unknown>).scores &&
      typeof (valuesJson as Record<string, unknown>).scores === "object"
        ? ((valuesJson as Record<string, unknown>).scores as Record<
            string,
            unknown
          >)
        : valuesJson;

    const entries = Object.entries(valueSource).filter(
      ([, value]) => typeof value === "number"
    );
    if (!entries.length) return null;
  ...
  const emotionJson = useMemo(
    () =>
      safeParseJson<Record<string, number | string>>(
        analysisData?.emotions ?? profileData?.emotions
      ),
    [analysisData, profileData]
  );
  ...
  const mbtiTraits = useMemo(() => {
    const personality = safeParseJson<Record<string, number>>(
    ...
    return [
      {
        ...
        explanation:
          extraversion >= 0.5
            ? "외향성 점수가 높아 E로 분류되었습니다."
            : "외향성 점수가 낮아 I로 분류되었습니다.",
        score: extraversion,
      },
      {
        pair: "N / S",
      ...
      <div className={styles["glass-card"]}>
        ...
        <div className="mb-5">
          <p className="text-lg font-bold text-indigo-600 mb-3">
            {analysisData?.mbti
              ? `"${analysisData.mbti} 유형의 잠재력을 가진 인재"`
              : '"분석 중..."'}
          </p>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 transition-colors shadow-lg"
          >
            상세 리포트 보기
          </button>
          <button
            onClick={() => setActiveTab("roadmap")}
            className="px-5 py-2.5 bg-white/80 text-slate-700 rounded-xl text-sm font-bold hover:bg-white transition-colors border border-slate-200 shadow-sm"
          >
            로드맵 생성하기
          </button>
        </div>
          ...
          <div className="space-y-3">
            {topJobs.length > 0 ? (
              topJobs.map((job) => (
                <div
                  key={job.rank}
                  className="flex items-center justify-between p-3 rounded-2xl bg-white/20 hover:bg-white/40 transition-all cursor-pointer group border border-white/30"
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-10 h-10 rounded-xl bg-gradient-to-br ${job.color} flex items-center justify-center text-white font-bold shadow-lg`}
                    >
                      {job.rank}
                    </div>
                    <div>
                      <p className="font-bold text-slate-800 group-hover:text-indigo-700 text-sm">
                        {job.title}
                      </p>
            ...
            {topMajors.length > 0 ? (
              topMajors.map((major) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-10 h-10 rounded-xl bg-gradient-to-br ${major.color} flex items-center justify-center text-white font-bold shadow-lg`}
                    >
                      {major.rank}
                    </div>
                    <div>
                      <p className="font-bold text-slate-800 group-hover:text-green-700 text-sm">
                        {major.title}
                      </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-indigo-100 mb-4 max-w-2xl text-sm">
                AI가 현재 상태를 분석하여 목표 달성을 위한 단계별 가이드를
                제공합니다. 지금 바로 시작해보세요.
              </p>
              <button
                onClick={() => setActiveTab("roadmap")}
                className="px-5 py-2.5 bg-white text-indigo-600 rounded-xl text-sm font-bold hover:bg-indigo-50 transition-colors shadow-lg"
              >
                로드맵 생성 시작하기
              </button>
            </div>
          ...
          <div className={styles["glass-card"]}>
            ...
            <div className="mt-4 space-y-3">
              {mbtiTraits.map((trait) => (
                <div
                  key={trait.pair}
                  className="rounded-lg border bg-gray-50/50 px-4 py-3"
                >
                  <div className="flex items-center justify-between">
  ...
  const renderValuesSection = () => (
    <div className="space-y-6">
      <ValuesSummaryCard
        valuesJSON={analysisData?.values ?? profileData?.values}
      />
      <div className={styles["glass-card"]}>
        <h3 className="text-lg font-semibold text-gray-800">
          가치관 집중도 (차트)
    ...
    <div className="space-y-6">
      ...
      <div className={styles["glass-card"]}>
        ...
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <button
            onClick={() => setActiveTab("roadmap")}
            className="px-6 py-3 bg-gradient-to-r from-pink-400 to-pink-500 text-white rounded-xl text-sm font-bold hover:opacity-90 transition-opacity shadow-lg"
          >
            학습 시작하기
          </button>
        </div>
      ...
      <div className="bg-gradient-to-r from-purple-500 to-indigo-600 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <button
            ...
            className="px-6 py-3 bg-white text-purple-600 rounded-xl text-sm font-bold hover:bg-purple-50 transition-colors shadow-lg"
          >
            멘토 신청하기
          </button>
        </div>
    ...
    <div className="space-y-6">
      <div className={styles["glass-card"]}>
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            disabled
            className="px-4 py-2 text-sm text-pink-600 hover:text-pink-700 disabled:text-gray-400 disabled:cursor-not-allowed flex items-center gap-1"
          >
            <FileText size={16} />
            수정하기
          </button>
    ...
    <>
      ...
      <div className={styles["glass-container"]}>
        ...
        <div className={styles["glass-box"]}>
          {/* Left Sidebar */}
          <div
            className={`${styles["glass-sidebar"]} ${
            ...
            <header className="h-20 flex items-center justify-between px-8 border-b border-white/10">
              <div>
                ...
                <p className="text-sm text-slate-600">
                  Welcome back! Here is your AI career analysis.
                </p>
              </div>
      ...
      {!collapsed && badge && (
        <span
          className={`px-2 py-0.5 text-[10px] font-bold rounded-full ${
            active ? "bg-blue-400 text-white" : "bg-indigo-100 text-indigo-600"
          }`}
        >
          {badge}
        </span>
