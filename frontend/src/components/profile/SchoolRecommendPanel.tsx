...
const SchoolRecommendPanel = ({ embedded = false }: Props) => {
  ...
  const handleFetch = async () => {
    ...
    setError(null);
    try {
      const res = await api.post("/recommend/schools", { vectorId: vectorId.trim() });
          ...
          {items.map((item, idx) => (
            <div key={`${item.id ?? idx}`} className="rounded-2xl border p-5 shadow-sm">
              ...
              <p className="mt-3 text-sm text-gray-600">
                {item.metadata?.desc || item.metadata?.description || "상세 설명이 준비 중입니다."}
              </p>
            </div>
          ))}
        </div>
