...
# ============== 6. 채용 공고 + 기술/자격증 통합 추천 ==============

class RequiredTechnology(BaseModel):
    """필요 기술"""
    name: str
    category: str  # 프로그래밍 언어, 프레임워크, 도구, 데이터베이스 등
    importance: str  # 필수, 우대, 권장
    description: Optional[str] = None
...
class RequiredCertification(BaseModel):
    ...
    estimatedPrepTime: Optional[str] = None  # 예상 준비 기간
    description: Optional[str] = None
...
class JobWithRequirements(BaseModel):
    ...
    skillGap: List[str] = []  # 사용자에게 부족한 스킬
...
class JobWithRequirementsRequest(BaseModel):
    ...
    careerAnalysis: Dict
    userProfile: Optional[Dict] = None
    userSkills: Optional[List[str]] = []  # 사용자 보유 스킬
    limit: int = Field(default=10, ge=1, le=50)
...
class JobWithRequirementsResponse(BaseModel):
    ...
    totalCount: int
    commonRequiredTechnologies: List[RequiredTechnology] = []  # 공통 필요 기술
    commonRequiredCertifications: List[RequiredCertification] = []  # 공통 필요 자격증
    overallLearningPath: List[str] = []  # 전체 학습 경로 추천
